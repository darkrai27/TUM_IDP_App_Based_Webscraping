...
from threadscraper.userSchemas import User
...
def get_followers(username: int, delay: int = 5, dtsg: str = None, session_id: str = None) -> Json:
  ...
  # count = 20
  # while (count < n or n == -1): #and response["data"]["followers"]["page_info"]["has_next_page"]:
  #   data['variables'] = f'{{"userID":"{user_id}","after":{count}, "count":20, "first":{count}}}'
  #   response = requests.post('https://www.threads.net/api/graphql', cookies=cookies, headers=headers, data=data)
  #   response = json.loads(response.text)
  #   res["data"]["followers"]["edges"].extend(response["data"]["followers"]["edges"])
  #   count += 10
  #   sleep(delay)

  # print(len(res["data"]["followers"]["edges"]))
...
def get_following(username: int, n: int = 100, delay: int = 5, dtsg: str = None, session_id: str = None) -> Json:
  ...
  '''
  Collects all (or n) accounts being followed by the user.

  Args:
    postID (int): Unique numerical identifier of a post.
    n (int): Maximum amount of people following to return.
    dtsg (str): Value generated by Meta to validated the session.
    session_id (str): Cookie identifier for the user session.
    delay (int): Delay among requests to avoid the account of getting suspended by bot activity.
  ...
  count = 20
  while count < n and response["data"]["following"]["page_info"]["has_next_page"]:
    data['variables'] = f'{{"userID":"{user_id}","after":{count}, "count":20, "first":10}}'
    response = requests.post('https://www.threads.net/api/graphql', cookies=cookies, headers=headers, data=data)
    response = json.loads(response.text)
    res["data"]["following"]["edges"].extend(response["data"]["following"]["edges"])
    count += 10
    sleep(delay)

  print(len(res["data"]["following"]["edges"]))

  return res

def get_user_posts(username: str, mode: str, n: int = 100, delay: int = 5, dtsg: str = None, session_id: str = None) -> Json:
  '''
  Returns all posts, replies or reposts or the most recent ones from the user up to a specified limit.
